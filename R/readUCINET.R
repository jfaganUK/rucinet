...
readUCINET <- function(fileName) {
  ...
  ## Replace NA
  # huge huge numbers are considered NA
  i <- which(f > 1e36)
  f[i] <- NA
  ...
  if(!is.null(ucinet.details$dim3)) {
    ...
    for(d3 in 1:ucinet.details$dim3) {
      ...
      m <- matrix(f[i0:i1], ncol=ucinet.details$dim1, nrow=ucinet.details$dim2, byrow = T)
    ...
    m <- matrix(f, ncol=ucinet.details$dim1, nrow=ucinet.details$dim2, byrow = T)
    colnames(m) <- ucinet.details$col.labels
    rownames(m) <- ucinet.details$row.labels
